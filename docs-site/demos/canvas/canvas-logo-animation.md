...
<DemoPreview
  demoUrl="/canvas/logo-animation/index.html"
  title="Logo 动画演示"
  description="Canvas Logo 动画效果，适合作为品牌开场或加载动画参考。"
/>
...
Canvas Logo 动画效果，适合作为品牌开场或加载动画参考。

## 为什么值得看

- Canvas Logo 动画效果，适合作为品牌开场或加载动画参考。
- 它把 Canvas / Logo / Animation 这类实现方式放在一个很容易上手的案例里。

## 技术要点

- 分类：Canvas 动画
- 标签：Canvas / Logo / Animation
- 场景：未补充
- 难度：高级
- 原始入口：
...
- [查看 GitHub 源码目录](https://github.com/Fridolph/my-program/tree/master/public/canvas/logo-animation)
- [返回 Canvas 动画 分类](../)
...
- 如果你正在做 Canvas 动画 相关页面，可以先把它当成结构和反馈节奏的参考。
- 如果你只需要快速验证某种交互感觉，也适合先从这个案例抽最小可复用部分。

## 实现拆解

- 先看 Canvas 与 Logo 在这个案例里分别承担了什么角色。
...
<div class="browse-chip-list"><a class="browse-chip" href="/explore/tags/canvas">Canvas</a><a class="browse-chip" href="/explore/tags/logo">Logo</a><a class="browse-chip" href="/explore/tags/animation">Animation</a></div>
...
<div class="browse-nav-grid">
    ...
    <a class="browse-nav-card" href="/demos/canvas/canvas-walking-dog">
      <span>下一条</span>
      <strong>Walking Dog</strong>
      <small>Walking Dog 动画演示，适合参考逐帧精灵与步行动作实现。</small>
    </a></div>
